...
const CharacterDetails = () => {
  ...
  useEffect(() => {
    const index = characters.findIndex((character) => character.id.toString() === characterId);
  ...
  const handlePreviousCharacter = useCallback(() => {
    const previousCharacterIndex = (characterIndex - 1 + characters.length) % characters.length;
  ...
  const featuredSeries = series.items.slice(0, 4);
    ...
    return (
        <div className="character-details">
            ...
            <Link className="back-link" to="/characters"><p>Go back</p></Link>
            <div className="navigation-icons">
                <FaRegHandPointLeft onClick={handlePreviousCharacter} className="arrow-icon left-arrow" />
                <FaRegHandPointRight onClick={handleNextCharacter} className="arrow-icon right-arrow" />
            </div>
